Feature: Member registration and loging
  ...
  Scenario: Any member must be registerd and logged in by simple signup to event. 
    ...
    When I go to "/calendarevents/nearest" with "user.email=new_user@email.email&user.first_name=userFirstName&user.last_name=userLastName&sponsor.email=4success.bz@gmail.com&sponsor.first_name=sName&sponsor.last_name=sLastName" placeholders
    Then I should see 3 ".day-webinar" elements
    Then I should have member with id "new_user@email.email"
     And member "4success.bz@gmail.com" should be sponsor
     And member "new_user@email.email" should be user
     And sponsor of "new_user@email.email" member should be "4success.bz@gmail.com"
    When I go to "/calendarevents/nearest" with "user.email=another_new_user@email.email&user.first_name=userFirstName&user.last_name=userLastName&sponsor.email=4success.bz@gmail.com&sponsor.first_name=sName&sponsor.last_name=sLastName" placeholders
    Then I should have member with id "another_new_user@email.email"
     And member "4success.bz@gmail.com" should be sponsor
     And member "another_new_user@email.email" should be user
     And sponsor of "another_new_user@email.email" member should be "4success.bz@gmail.com"