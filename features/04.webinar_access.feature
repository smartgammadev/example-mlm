Feature: User access to closed webinars must be granted or denied by {{user.businessLinkFull}}
  ...
  Scenario: Create a closed event and check access for it in a frontend
    ...
    Given I am not logged
     When I go to "/calendarevents/nearest" with "user.email=user@mail&user.first_name=uName&user.last_name=uLastName&sponsor.email=4success.bz@gmail.com&sponsor.first_name=sName&sponsor.last_name=sLastName&user.businessLinkFull=http://example.com" placeholders
